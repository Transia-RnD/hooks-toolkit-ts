...
describe('escrowCancel', () => {
  ...
  it('txn escrow cancel id', async () => {
    // set up hook
    const hook = createHookPayload(
      0,
      'txn_escrow_cancel_id',
      'txn_escrow_cancel_id',
    ...
    expect(hookExecutions.executions[0].HookReturnString).toMatch(
      'txn_escrow_cancel_id.c: Tx emitted success.'
    )
    await close(testContext.client)
  ...
  it('txn escrow cancel seq', async () => {
    // set up hook
    const hook = createHookPayload(
      0,
      'txn_escrow_cancel_seq',
      'txn_escrow_cancel_seq',
      SetHookFlags.hsfOverride,
      ['Invoke']
    )
    ...
    expect(hookExecutions.executions[0].HookReturnString).toMatch(
      'txn_escrow_cancel_seq.c: Tx emitted success.'
    )
    await close(testContext.client)
    // confirm that the offer actually went through
    const accountObjectsResponse2 = await testContext.client.request({
