...
int64_t hook(uint32_t reserved) {
    ...
    int64_t amount_native = hook_param(SBUF(amount_buf), SBUF(amount_key)) == 8;

    int64_t amount = *((int64_t*)amount_buf);
    TRACEVAR(amount);
    if (float_compare(amount, 0, COMPARE_LESS | COMPARE_EQUAL) == 1)
        rollback(SBUF("txn_payment.c: Invalid Txn Parameter `AMT`"), __LINE__);

    // ACCOUNT: To Account
    uint8_t to_key[2] = { 'T', 'O' };
    ...
    *((uint32_t*)(LLS_OUT)) = FLIP_ENDIAN(lls);
    ...
    if (emit_result > 0)
    {
        accept(SBUF("txn_payment.c: Tx emitted success."), __LINE__);
    }
    accept(SBUF("txn_payment.c: Tx emitted failure."), __LINE__);
