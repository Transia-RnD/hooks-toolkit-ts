...
uint8_t txn[295] =
    {
        ...
        /*  49, 30  */ 0x64U, 0x99U, 0x99U, 0x99U, 0x99U, 0x99U, 0x99U, 0x99U,                          /* amount field 9 or 49 bytes */
...
int64_t hook(uint32_t reserved) {
    ...
    TRACESTR("txn_uritoken_buy.c: Called.");
    ...
    {
        rollback(SBUF("txn_uritoken_buy.c: Invalid Txn Parameter `ID`"), __LINE__);
    }
    ...
    if (float_compare(amount, 0, COMPARE_LESS | COMPARE_EQUAL) == 1)
        rollback(SBUF("txn_uritoken_buy.c: Invalid Txn Parameter `AMT`"), __LINE__);
    ...
    *((uint32_t*)(LLS_OUT)) = FLIP_ENDIAN(lls);
    ...
    if (emit_result > 0)
    {
        accept(SBUF("txn_uritoken_buy.c: Tx emitted success."), __LINE__);
    }
    accept(SBUF("txn_uritoken_buy.c: Tx emitted failure."), __LINE__);
