...
int64_t hook(uint32_t r)
{
    ...
    if (BUFFER_EQUAL_20(hook_acc, otxn_acc))
        accept(SBUF("xpop_nft_uri.c: Passing outgoing txn."), __LINE__);

    if (otxn_type() != ttIMPORT)
        accept(SBUF("xpop_nft_uri.c: Passing non ttIMPORT txn."), otxn_type());

    int64_t retval = xpop_slot(1,2);

    if (retval <= 0)
        NOPE("xpop_nft_uri.c: Failed to slot xpop");
...
#define meta_slot 2
    ...
    if (slot_subfield(meta_slot, sfTransactionResult, 3) != 3)
        NOPE("xpop_nft_uri.c: Failed to slot transaction result");

    uint8_t tr;
    if (slot(SVAR(tr), 3) != 1)
        NOPE("xpop_nft_uri.c: Failed to dump transaction result");

    trace_num(SBUF("Inner Transaction Result:"), tr);
    if (tr != 0)
        NOPE("xpop_nft_uri.c: Inner Transaction Result not tesSUCCESS (0).");

    // execution to here means tesSUCCESS on inner

    if (slot_subfield(tx_slot, sfTransactionType, 4) != 4)
        NOPE("xpop_nft_uri.c: Could not slot transaction type");

    uint8_t tt_buf[2];
    if (slot(SBUF(tt_buf), 4) != 2)
        NOPE("xpop_nft_uri.c: Could not dump transaction type");

    uint16_t tt = UINT16_FROM_BUF(tt_buf);

    if (tt != ttNFTOKEN_BURN)
        NOPE("xpop_nft_uri.c: Only NFTokenBurn is accepted");
...
#define nodes 5
    if (slot_subfield(meta_slot, sfAffectedNodes, nodes) != nodes)
       NOPE("xpop_nft_uri.c: Could not slot sfAffectedNodes");
    ...
    for (int i = 0; GUARD(5), i < count; ++i)
    {
        ...
        if (slot_subfield(6, sfLedgerEntryType, 7) != 7)
            NOPE("xpop_nft_uri.c: Could not slot LedgerEntryType");
    ...
    if (!found)
        NOPE("xpop_nft_uri.c: Could not find NFTokenPage in xpop metadata");

    if (slot_subfield(6, sfPreviousFields, 6) != 6 && slot_subfield(6, sfFinalFields, 6) != 6)
        NOPE("xpop_nft_uri.c: Could not slot sfPreviousFields");

    if (slot_subfield(6, sfNFTokens, 6) != 6)
        NOPE("xpop_nft_uri.c: Could not slot sfNFTokens");

    count = slot_count(6);
    if (count > 32) count = 32;

    trace_num(SBUF("xpop_nft_uri.c: Modified node count:"), count);
    uint8_t burned_tid[32];
    if (slot_subfield(tx_slot, sfNFTokenID, 7) != 7 || slot(SBUF(burned_tid), 7) != 32)
        NOPE("xpop_nft_uri.c: Xpop txn did not contain valid nftokenid");
    ...
    if (!found)
        NOPE("xpop_nft_uri.c: Could not find the NFTokenID in the metadata");

    trace(SBUF("xpop_nft_uri.c: URI from xpop: "), uri, urilen, 1);
    ...
    trace(SBUF("xpop_nft_uri.c: emit txn"), txn_buf, txn_len, 1);

    uint8_t etxid[32];
    if (emit(SBUF(etxid), txn_buf, txn_len) < 0)
        NOPE("Emission failed");

    trace(SBUF("xpop_nft_uri.c: Emission success"), etxid, 32, 1);
    return accept(SBUF("xpop_nft_uri.c: Finished"), __LINE__);
}